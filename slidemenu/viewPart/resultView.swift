...
struct resultView: View {
    ...
    @State var switch_test = Array(repeating: false, count: 4)
    var body: some View {
                    ...
                    ForEach(0..<data.result!.count){ i in
                        if let des=data.result?[i].baike_info?.description,let img=data.result?[i].baike_info?.image_url{
                            cell2(img_url: img, des: des,score: data.result![i].score, name: data.result![i].name)
                                ...
                                .sheet(isPresented: self.$switch_test[i]) {
                                    //SafariView(url:data.result![i].baike_info.baike_url)
                                    textView(name:data.result![i].name,str:des,url:img)
                                }
                                .onTapGesture {
                                    switch_test[i]=true
                                }
